...
const BlogPostSummary = (props) => {
  let fluidImg = props.node.frontmatter.coverImage ? (
    ...
    <div className="w-100 h-56 md:h-48 lg:h-40 text-center flex items-center"
  ...
  return (
      <article key={props.node.fields.slug} className="p-2 relative w-full sm:w-full md:w-1/2 lg:w-1/3 md:h-90">
        <Link to={link}>
          {fluidImg}
        </Link>
